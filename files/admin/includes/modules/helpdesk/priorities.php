...
$config = array(
			'primary_field' => 'priority_id',
			'id' => $_GET['dId'],
			'table' => TABLE_HELPDESK_TICKETS_PRIORITY,
			'prefix' => 'priority',
			'language_field'=> 'languages_id',
			'fields'		=> array(
									'priority_name' 	=> array('type' => 'varchar', 'multilingual' => true,  'lable' => HELPDESK_TEXT_PRIORITY_NAME),
									'priority_color' 	=> array('type' => 'varchar', 'multilingual' => false,  'lable' => HELPDESK_TEXT_PRIORITY_COLOR),
									'sort_order' 	=> array('type' => 'integer', 'multilingual' => false, 'lable' => HELPDESK_TEXT_PRIORITY_SORT_ORDER)
								)
		);
...
switch($_GET['action'])
{
	case 'save_sort':
		if(isset($_POST['sort_order']))
		{
			...
			{
				$sql = 'UPDATE '.TABLE_HELPDESK_TICKETS_PRIORITY	.' SET sort_order = \'' . xtc_db_input($value) . '\' WHERE priority_id = \'' . xtc_db_input($id) . '\'';
				xtc_db_query($sql);
			}
		}
		break;
	...
	case 'save':
		if(isset($_POST['save']))
		{
			saveMultiLingualContent($config, $_POST['priority']);
		}
		break;
	...
	case 'delete':
		...
		{
			$sql = 'DELETE FROM ' . TABLE_HELPDESK_TICKETS_PRIORITY . ' WHERE priority_id = \'' . xtc_db_input($_GET['dId']) . '\'';
			xtc_db_query($sql);
			xtc_redirect(xtc_href_link($filename, xtc_get_all_get_params( array('dId', 'action') ) ));
		}
		break;
	...
	switch($_GET['action'])
	{
		...
		case 'list':
			...
			echo '<table class="listing">';
				echo '<tr>';
					echo '<th>' . HELPDESK_TEXT_PRIORITY_NAME . '</th>';
					echo '<th>' . HELPDESK_TEXT_PRIORITY_SORT_ORDER . ' <input type="submit" name="save_sort" value="'.BUTTON_SAVE.'" /></th>';
					echo '<th>' . HELPDESK_TEXT_PRIORITY_COLOR . '</th>';
			...
			while($row = xtc_db_fetch_array($rs))
			{
				echo '<tr  style="background-color:#'.$row['priority_color'].';">';
					...
					echo '<td><input type="text" name="sort_order['.$row['priority_id'].']" style="text-align:center;" size="3" value="'.$row['sort_order'].'" /></td>';
					echo '<td>'.$row['priority_color'].'</td>';
					echo '<td><a href="'.xtc_href_link($filename, xtc_get_all_get_params(array('dId', 'action')).'&action=edit&dId='.$row['priority_id'] ) . '">'.BUTTON_EDIT.'</a></td>';
					echo '<td><a href="'.xtc_href_link($filename, xtc_get_all_get_params(array('dId', 'action')).'&action=delete&dId='.$row['priority_id'] ) . '">'.BUTTON_DELETE.'</a></td>';
				echo '</tr>';
			}
			
			echo '</table>';
			echo '<br />';
			echo '<a href="'.xtc_href_link($filename, xtc_get_all_get_params(array('dId', 'action')).'&action=edit&dId=new' ) . '">'.BUTTON_NEW_PRIORITY.'</a>';
			echo '</form>';
			break;
		...
		case 'delete':
			...
			echo HELPDESK_TEXT_PRIORITY_DELETE_CONFIRM;
